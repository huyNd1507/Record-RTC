...
//         <div>
//           {recordingType !== "full-screen" && isRecording && (
//             <video
//               src=""
//               ref={(video) => {
...
export default function Home() {
  ...
  const [isRecording, setIsRecording] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [recordDuration, setRecordDuration] = useState<number | null>(null); // Thêm state để lưu thời gian ghi
  ...
  const startRecording = async () => {
    try {
      ...
      if (recordDuration) {
        setTimeout(() => {
          stopRecording();
        }, recordDuration * 1000); // Chuyển đổi từ giây sang mili giây
      }
    } catch (error) {
      console.error("Error starting recording:", error);
    }
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        ...
        {/* Thêm Select để chọn thời gian ghi */}
        <div className="flex items-center gap-2">
          <Select
